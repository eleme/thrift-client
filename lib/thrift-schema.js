...
class ThriftSchema {
  ...
  encodeStruct(schema, params = {}) {
    ...
    for (let i = 0; i < schema.length; i++) {
      ...
      let value = params[name] || schema[i].defaultValue;
      if (value !== void 0) {
  ...
  decodeStruct(schema, value) {
    ...
    for (let i = 0; i < schema.length; i++) {
      let name = schema[i].name;
      let value = sMap[schema[i].id] || schema[i].defaultValue;
      if (schema[i].option === 'required' && value === void 0) {
        throw new ThriftSchemaMismatchResponse('Required field "' + name + '" not found');
      }
      if (value !== void 0) result[name] = this.decodeValueWithType(value, schema[i].type);
    }
    return result;
  }
